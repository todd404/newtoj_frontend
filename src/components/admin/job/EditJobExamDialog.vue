...
const jobExamList = ref<JobExam[]>([])
const examEditTableData = computed<ExamEditTableItem[]>(() =>
  examList.value.map((v) => {
    let isSelected = true
    if (jobExamList.value.find((value) => value.examId == v.id.toString())) {
      isSelected = true
    } else {
      isSelected = false
    }
...
const getCourseExam = async () => {
  const res = await axios.get(`/api/job-exam-list`, { params: { jobId: props.jobId } })
  const data: ResponseResult = res.data
  if (data.code != 200) {
    showErrorMessge('获取测验信息失败')
  } else {
    jobExamList.value = data.data
  }
}
...
const handleExamSelectClick = async (examId: number) => {
  ...
  const res = await axios.post(`/api/add-job-exam`, req)
  const data: ResponseResult = res.data
  if (data.code != 200) {
...
const handleExamUnSelectClick = async (examId: number) => {
  ...
  const res = await axios.post(`/api/delete-job-exam`, req)
  const data: ResponseResult = res.data
  if (data.code != 200) {
